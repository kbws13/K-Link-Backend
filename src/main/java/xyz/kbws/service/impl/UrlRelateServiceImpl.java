...
import xyz.kbws.constant.ShortLinkConstant;
...
import xyz.kbws.model.enums.ShortLinkStatusEnum;
...
import xyz.kbws.mapper.UrlRelateMapper;
import org.springframework.stereotype.Service;
import xyz.kbws.service.UrlTagService;
import xyz.kbws.service.UserService;
import xyz.kbws.utils.*;
...
public class UrlRelateServiceImpl extends ServiceImpl<UrlRelateMapper, UrlRelate>
    ...
    public boolean updateUrlRelate(UrlRelateUpdateRequest urlRelateUpdateRequest) {
        ...
        //移除缓存
        RMap<String, UrlRelate> shortLinkMap = redisson.getMap(RedissonConstants.SHORT_LINK_CACHE_MAP_KEY);
    ...
    private String createShortUrl(String longUrl) {
        ...
        // 使用布隆过滤器校验
        RBloomFilter<String> bloomFilter = redisson.getBloomFilter(RedissonConstants.SHORT_LINK_BLOOM_FILTER_KEY);
        //初始化布隆过滤器：预计元素为100000000L,误差率为3%
        bloomFilter.tryInit(100000000L, 0.03);

        if (!bloomFilter.contains(shortUrlStr64)) {
            //hash冲突重新生成 在结尾重新添加一个分布式id
            shortUrlStr64 = shortUrlStr64 + distributedIdGenerator.generateDistributedId(ShortLinkConstant.SHORT_LINK);
        }
